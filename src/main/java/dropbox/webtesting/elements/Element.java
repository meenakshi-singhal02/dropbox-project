package dropbox.webtesting.elements;
...
public class Element {
	...
	public int getRowCount() {
		getElement();
		int count = element.findElements(By.xpath("li")).size();
		return count;
	}
	
	public void selectListCount(String sFileName)
	{
		int listCount = getRowCount();
		for(int iListCounter=1;iListCounter<=listCount;iListCounter++)
		{
			String sActualFileName= element.findElement(By.xpath("/li[iListCounter]/div/div/div[2]/div/a")).getText();
			
			if(sActualFileName.contains(sFileName))
			{
				element.findElement(By.xpath("/li[iListCounter]/div/div/div[3]/div/div[2]/button")).click();
			}		
		}
	}
}
